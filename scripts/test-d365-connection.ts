  ...
  try {
    ...
    if (ordersData.value?.length > 0) {
      ...
      for (const order of ordersData.value.slice(0, 3)) {
        console.log(
          `   - ${order.SalesOrderNumber} | ${order.THK_ShopifyReference || "No Shopify Ref"} | ${order.CurrencyCode}`
        );
      }
    }
      ...
      if (linesResponse.ok) {
        ...
        if (linesData.value?.length > 0) {
          ...
          for (const line of linesData.value.slice(0, 3)) {
            console.log(
              `   - ${line.ItemNumber} | Qty: ${line.OrderedSalesQuantity} | LotId: ${line.InventoryLotId || "N/A"}`
            );
          }
        }
      } else {
