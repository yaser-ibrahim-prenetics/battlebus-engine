...
console.log(
  `  Access Token: ${SHOPIFY_ACCESS_TOKEN ? "***" + SHOPIFY_ACCESS_TOKEN.slice(-8) : "NOT SET"}`
);
console.log(
  `  Webhook Secret: ${SHOPIFY_WEBHOOK_SECRET ? "***" + SHOPIFY_WEBHOOK_SECRET.slice(-8) : "NOT SET"}`
);
    ...
    if (ordersResponse.ok) {
      ...
      if (ordersData.orders?.length > 0) {
        ...
        for (const order of ordersData.orders.slice(0, 3)) {
          console.log(
            `   - #${order.order_number} | ${order.email} | ${order.financial_status} | ${order.fulfillment_status || "unfulfilled"}`
          );
        }
      }
    } else {
