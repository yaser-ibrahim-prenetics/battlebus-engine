...
const JWT_TOKEN =
  "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..a1VB0ZTMVD9r5n4mL1-Ap0KpGd1U7aoibMKzd-M7LDk";
...
async function main() {
  ...
  const endpoints = [
    ...
    {
      path: "/gateway/woms/report/stock/list",
      method: "POST" as const,
      body: { page: 1, pageSize: 10 },
    },
    {
      path: "/gateway/woms/report/stock/query",
      method: "POST" as const,
      body: { page: 1, pageSize: 10 },
    },
    {
      path: "/gateway/woms/inventory/list",
      method: "POST" as const,
      body: { page: 1, pageSize: 10 },
    },
    {
      path: "/gateway/woms/inventory/query",
      method: "POST" as const,
      body: { page: 1, pageSize: 10 },
    },
    ...
    {
      path: "/gateway/woms/product/list",
      method: "POST" as const,
      body: { page: 1, pageSize: 10 },
    },
  ...
  for (const ep of endpoints) {
    ...
    if (result.status === 200 && result.data) {
      ...
      console.log(
        `❌ ${result.status} - ${JSON.stringify(result.data || result.error).slice(0, 100)}`
      );
    }
  }
