...
export async function sendOrderEvent(event: OrderEvent): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      await logFlowEvent({ flow: "cs_platform", client: "cs_platform", step: event.event, status: "failed", level: "error", durationMs: Date.now() - _start, errorMessage: error, shopifyOrderName: event.data?.shopifyOrderName });
      throw new Error(`CS Platform webhook failed: ${error}`);
    }

    console.log(`[CS Platform] ✅ Sent event: ${event.event}`);
    await logFlowEvent({ flow: "cs_platform", client: "cs_platform", step: event.event, status: "completed", durationMs: Date.now() - _start, shopifyOrderName: event.data?.shopifyOrderName });
  } catch (error) {
    console.error(`[CS Platform] Error sending event ${event.event}:`, error);
    await logFlowEvent({ flow: "cs_platform", client: "cs_platform", step: event.event, status: "failed", level: "error", durationMs: Date.now() - _start, errorMessage: error instanceof Error ? error.message : String(error), shopifyOrderName: event.data?.shopifyOrderName }).catch(() => {});
  }
}
