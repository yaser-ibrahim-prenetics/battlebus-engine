...
import { shopifyRefundCreatedByLoopReturns, shouldSuppressShopifyRefundWebhookForLoopReturns } from "../shopify-loop-refund-detection";
...
describe("shouldSuppressShopifyRefundWebhookForLoopReturns", () => {
  beforeEach(() => {
    vi.mocked(shopify.getOrderEvents).mockReset();
  });

  it("returns false when Loop integration is disabled", async () => {
    const refund: ShopifyRefundPayload = {
      id: 99,
      order_id: 2,
      created_at: "",
      refund_line_items: [],
      transactions: [{ id: 1, kind: "refund", status: "success", amount: "50" }],
    };

    await expect(
      shouldSuppressShopifyRefundWebhookForLoopReturns("2", refund, {
        loopIntegrationEnabled: false,
      })
    ).resolves.toBe(false);
    expect(shopify.getOrderEvents).not.toHaveBeenCalled();
  });

  it("delegates to order-events Loop detection when integration is enabled", async () => {
    vi.mocked(shopify.getOrderEvents).mockResolvedValue({
      events: [
        {
          id: 1,
