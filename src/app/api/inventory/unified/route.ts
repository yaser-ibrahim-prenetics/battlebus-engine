...
import { getInventory as getGpsInventory, getAllInventory as getAllGpsInventory } from "@/lib/clients/gps";
import { getInventory as getD365Inventory, getAllInventory as getAllD365Inventory } from "@/lib/clients/dynamics";
  ...
  try {
    ...
    if (sources.includes("gps")) {
      try {
        ...
        for (const item of gpsItems) {
          ...
          existing.totalAvailable = existing.sources.gps.available + (existing.sources.d365?.available || 0);
        ...
        errors.push({ source: "gps", error: error instanceof Error ? error.message : String(error) });
      }
    }
    ...
    if (sources.includes("d365")) {
      try {
        ...
        errors.push({ source: "d365", error: error instanceof Error ? error.message : String(error) });
      }
    }
